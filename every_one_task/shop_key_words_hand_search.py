...
class shop_key_words_hand_search:
    
    def __init__(self, config) -> None:
        self.base = base_action()
        self.task_name = "[店铺关键词][手淘搜索]"
        self.get_config_bool = self.base.get_configs('sycmShopKeyWordsHandSearch', config_name=config)
    ...
    def visit_sycm(self):
        ...
        if self.get_config_bool is False:
            print(f'{self.base.config_obj["shop_name"]}: <error> [手淘搜索] 读取配置文件出错，请检查。')
            return False
        
        if self.create_folder_bool is False:
            print(f'{self.base.config_obj["shop_name"]}: <error> [手淘搜索] 创建存储文件出错，请检查。')
            return False
        ...
        if res is False:
            print(f'{self.base.config_obj["shop_name"]}: <error> [手淘搜索] 访问生意参谋失败，请检查。')
            return False
        ...
        if res is False:
            print(f'{self.base.config_obj["shop_name"]}: <error> [手淘搜索] 登录失败，请检查！')
            return False
    ...
    def down_load_excel(self):
        ...
        for date in data_range:
            ...
            if self.data_bool:
                ...
                print(f'{self.base.config_obj["shop_name"]}: <error> [手淘搜索] 下载数据失败， 失败日期： {date_}')
            ...
            if is_byte:
                ...
                print(f'{self.base.config_obj["shop_name"]}: <error> [手淘搜索] 下载的数据格式不正确，请检查, {date_}')
                self.base.page.wait(random.randint(1, 3))
                continue
            
            # print(f' {date_} 的数据 下载完毕， 已保存！')
            self.base.page.wait(random.randint(1, 3))
        ...
        try:
            ...
            for filename in filelist:
                ...
                if res:
                    ...
                    print(f'{self.base.config_obj["shop_name"]}: <error> [手淘搜索] {filename} 写入失败')
            ...
            print(f'{self.base.config_obj["shop_name"]}: <error> [手淘搜索] {filename} 写入报错， 文件已剪切至 failure 文件夹！')
            print(e)
    ...
    def run(self):
        ...
        print(f'{self.base.config_obj["shop_name"]}: <info> 开始执行 [手淘搜索]!')
        ...
        print(f'{self.base.config_obj["shop_name"]}: <info> 执行完毕 [手淘搜索]!')
...
if __name__ == "__main__":
    test = shop_key_words_hand_search()
    test.run()