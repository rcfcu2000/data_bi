...
class commodity_keywords_train:
    
    def __init__(self, config) -> None:
        self.base = base_action()
        self.config = config
        self.task_name = "[单品分析][直通车]"
        self.get_config_bool = self.base.get_configs('sycmCommodityKeyWordsTrain', config_name=self.config)
        ...
        for date in date_range:
            ...
            for i in range(0, len(data)):
                ...
                if json_data['code'] == 0:
                    ...
                    self.log(msg=f'<error> [商品关键词][直通车] 访问数据失败&&访问日期&&{date}&&商品ID&&{data[i]}')
